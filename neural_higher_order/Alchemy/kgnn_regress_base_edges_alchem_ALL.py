...
class ZINCbase(InMemoryDataset):
    ...
    def raw_file_names(self):
        return "ZIfNC_alggfrdggfffdvvvvlkrfsvfffffsd0kdd"

    @property
    def processed_file_names(self):
        return "ZINfC_adfgsdffflggrffflfvfkffrdccf1s0kdd"
    ...
    def process(self):
        ...
        node_labels = kb.get_all_node_labels_1("alchemy_full", True)
        edge_labels = kb.get_all_edge_labels_1("alchemy_full")

        print("###")
        matrices = kb.get_all_matrices_1("alchemy_full", list(range(202579)))
...
class GINConv(MessagePassing):
    def __init__(self, emb_dim, dim1, dim2):
        ...
        self.bond_encoder = Sequential(Linear(emb_dim, dim1), torch.nn.BatchNorm1d(dim1),  ReLU(), Linear(dim1, dim1), torch.nn.BatchNorm1d(dim1), ReLU())
        ...
        self.mlp = Sequential(Linear(dim1, dim2),  torch.nn.BatchNorm1d(dim2), ReLU(), Linear(dim2, dim2), torch.nn.BatchNorm1d(dim2), ReLU())
...
for _ in range(5):
    ...
    dataset = ZINCbase(path, transform=MyTransform()).shuffle()
    print(len(dataset))
