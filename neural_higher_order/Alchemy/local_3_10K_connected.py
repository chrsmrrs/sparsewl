...
class Alchemy(InMemoryDataset):
    ...
    def process(self):
        ...
        matrices.extend(pre.get_all_matrices_3_connected("alchemy_full", indices_test))
...
for _ in range(5):
    ...
    def train():
        ...
        lf = torch.nn.L1Loss()
        lf_all = torch.nn.L1Loss(reduction="sum")
        for data in train_loader:
            ...
            loss.backward()
            loss_all += lf_all(model(data), data.y)
            optimizer.step()
